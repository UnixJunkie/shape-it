...
void
listAtomVolumes(OpenBabel::OBMol& mol, GaussianVolume& gv)
{
   ...
   for (OpenBabel::OBAtom* a = mol.BeginAtom(ai); a; a = mol.NextAtom(ai))
   {
		...
		if (a->GetAtomicNum() == 1)
      {
		...
		gv.gaussians[atomIndex].volume = (4.0 * PI/3.0) * radius * radius * radius;
		...
		gv.centroid.x += gv.gaussians[atomIndex].volume * gv.gaussians[atomIndex].center.x;
		gv.centroid.y += gv.gaussians[atomIndex].volume * gv.gaussians[atomIndex].center.y;
		gv.centroid.z += gv.gaussians[atomIndex].volume * gv.gaussians[atomIndex].center.z;
		...
		for (int i = 0; i < atomIndex; ++i)
      {
			...
			AtomGaussian ga = atomIntersection(gv.gaussians[i], gv.gaussians[atomIndex]);
			...
			if (ga.volume / (gv.gaussians[i].volume + gv.gaussians[atomIndex].volume - ga.volume) < EPS)
         {
			...
			parents.push_back(std::make_pair<unsigned int,unsigned int>(i, atomIndex));
	...
	for (unsigned int l = 2; l < LEVEL; ++l)
   {
		...
		for (unsigned int i = startLevel; i < nextLevel; ++i)
      {
			...
			std::set_intersection(overlaps[a1]->begin(),overlaps[a1]->end(),
                               overlaps[a2]->begin(),overlaps[a2]->end(),
			...
			if (overlaps[i]->empty())
         {
			...
			for (setIter = overlaps[i]->begin(); setIter != overlaps[i]->end(); ++setIter )
         {
				if (*setIter <= a2)
            {
					continue;
				...
				AtomGaussian ga = atomIntersection(gv.gaussians[i], gv.gaussians[*setIter]);
								
				// Check if the volume is large enough
				if (ga.volume / (gv.gaussians[i].volume + gv.gaussians[*setIter].volume - ga.volume) < EPS)
            {
					continue;
				}
	...
	for (std::vector<std::set<unsigned int> *>::iterator si=overlaps.begin(); si != overlaps.end(); ++si)
   {
		if (*si != NULL)
      {
			delete *si;
...
void
initOrientation(GaussianVolume& gv)
{
	...
	for (std::vector<AtomGaussian>::iterator i = gv.gaussians.begin(); i != gv.gaussians.end(); ++i)
   {
		// Translate to center
	...
	for (std::vector<AtomGaussian>::iterator i = gv.gaussians.begin(); i != gv.gaussians.end(); ++i)
   {
		...
		y = i->center.y;
		z = i->center.z;
		i->center.x = gv.rotation[0][0] * x + gv.rotation[1][0] * y + gv.rotation[2][0] * z;
		i->center.y = gv.rotation[0][1] * x + gv.rotation[1][1] * y + gv.rotation[2][1] * z;
		i->center.z = gv.rotation[0][2] * x + gv.rotation[1][2] * y + gv.rotation[2][2] * z;
	}
...
double 
atomOverlap(GaussianVolume& gRef, GaussianVolume& gDb)
{
	...
	for (unsigned int i(0); i < N1; ++i)
   {
		for (unsigned int j(0); j < N2; ++j)
      {
			...
			Vij = gRef.gaussians[i].C * gDb.gaussians[j].C * pow(PI/(gRef.gaussians[i].alpha + gDb.gaussians[j].alpha),1.5) * exp(- Cij * (dx+dy+dz) );

			// Check if overlap is sufficient enough
			if (Vij/(gRef.gaussians[i].volume + gDb.gaussians[j].volume - Vij) < EPS)
         { 
				continue;
         }
	...
	while(!processQueue.empty())
	{
		...
		Vij = gRef.gaussians[i].C * gDb.gaussians[j].C * 
            pow(PI/(gRef.gaussians[i].alpha + gDb.gaussians[j].alpha),1.5) * exp(- Cij * (dx+dy+dz) );
...
double
getScore(std::string& id, double Voa, double Vra, double Vda)
{
...
void
checkVolumes(GaussianVolume& gRef, GaussianVolume& gDb, AlignmentInfo& res)
{
